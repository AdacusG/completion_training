...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate the accuracy of a sequence completion transformer model."
    )
    
    # Optional positional argument for the evaluation file
    parser.add_argument(
        "eval_file_path",
        nargs="?",
        default=None,  # Handled dynamically below to ensure data_dir fallback works
        help="Path to the file to evaluate (default: data_dir/input.txt)"
    )
    
    # Optional flags for directories and model configuration
    parser.add_argument(
        "--out_dir",
        type=str,
        default="out_1char",
    ...
    # Dynamically determine evaluation file path if not explicitly provided
    eval_file_path = args.eval_file_path if args.eval_file_path else os.path.join(data_dir, 'input.txt')
